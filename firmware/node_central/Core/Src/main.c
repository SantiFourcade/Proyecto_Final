...
int main(void)
{
    ...
    /* 2. Activar AFIO para Remap PB8/PB9 */
    ...
    /* 4. Configurar Filtro y Arrancar CAN */

    if (HAL_CAN_Start(&hcan) != HAL_OK) {
        printf("Error iniciando CAN\r\n");
        Error_Handler();
    }

    /* 5. Activar Notificaciones de Interrupción */
    if (HAL_CAN_ActivateNotification(&hcan, 
        CAN_IT_RX_FIFO0_MSG_PENDING | CAN_IT_ERROR) != HAL_OK) {
        printf("Error activando IT CAN\r\n");
        Error_Handler();
    }
    uint32_t error = HAL_CAN_GetError(&hcan);
    printf("Código de error CAN: 0x%08lX\r\n", error);

    uint32_t level = HAL_CAN_GetRxFifoFillLevel(&hcan, CAN_RX_FIFO0);
    printf("Mensajes en FIFO: %lu\r\n", level);

    printf("\r\n=== NODO CENTRAL: CAN + SD LOGGER READY ===\r\n");
...
/* --- Funciones de soporte (Filtro, Printf, Reloj) --- */
...
void SystemClock_Config(void)
{
  ...
  RCC_OscInitStruct.PLL.PLLMUL = RCC_PLL_MUL4; // 8MHz * 4 = 32MHz para sincronizar
