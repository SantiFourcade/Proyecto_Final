...
void HAL_CAN_RxFifo0MsgPendingCallback(CAN_HandleTypeDef *hcan)
{
    ...
    if (HAL_CAN_GetRxMessage(hcan, CAN_RX_FIFO0, &rxHeader, rxData) == HAL_OK)
    {
        ...
        // El truco de la memoria para reconstruir el float
        memcpy(&value, rxData, sizeof(float));

        // Identificamos por ID
        char* label = "DATA";
        if(rxHeader.StdId == 0x101) label = "RMS";
        else if(rxHeader.StdId == 0x102) label = "CREST";
        else if(rxHeader.StdId == 0x103) label = "PEAK";

        printf("[RX] %s (0x%03lX): %.3f\r\n", label, rxHeader.StdId, value);
    }
}
